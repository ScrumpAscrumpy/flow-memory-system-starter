...
这个项目本身是为了让 Codex 以及类似的 AI coding agent 更方便地管理文件、流程和任务上下文而设计的。它不会把你的项目数据上传到某个云端服务，也不依赖远程账号体系才能使用。整套系统默认运行在你的本地项目目录里。

需要说明的是：项目在本地仍然可能生成一些你自己可见、可控制的文件，比如 `flows/` 文档或 `flow_memory_logs/` 日志。这些内容默认只留在你的机器上，除非你主动提交、同步或分享出去。

当前这个仓库主打的是 macOS 桌面使用体验。虽然 Python 源码本身可以继续适配其他平台，但目前打磨好的桌面启动方式暂时只有 macOS 版本。
